...
void model_pushNormals(Model* model, const Vec3_Array normals){
        if(model->normals.size == 0){
                model->normals.array = (vec3*)malloc(sizeof(vec3)*normals.size);
        }else{
                model->normals.array = (vec3*)realloc(model->normals.array, sizeof(vec3)*(model->normals.size+normals.size));
        }
        for(int i = 0; i < normals.size; i++){
                glm_vec3_copy(normals.array[i], model->normals.array[model->normals.size]);
                model->normals.size++;
        }
}
...
void model_pushFaces(Model* model, const u32_Array faces){
        if(model->faces.size == 0){
                model->faces.array = (u32*)malloc(sizeof(u32)*faces.size*3);
        }else{
                model->faces.array = (u32*)realloc(model->faces.array, sizeof(u32)*(model->faces.size+faces.size)*3);
        }
        //printf("%d faces\n\n", faces.size);
        for(int i = 0; i < faces.size; i++){
                model->faces.array[model->faces.size*3]   = faces.array[i*3];
                model->faces.array[model->faces.size*3+1] = faces.array[(i*3)+1];
                model->faces.array[model->faces.size*3+2] = faces.array[(i*3)+2];
                model->faces.size++;
        }
}
...
void model_subdivideFace(Model* model, u32 face){
        ...
        Vec3Array_Push(&normals, (vec3){0.0f, 0.0f, 0.0f});
        Vec3Array_Push(&normals, (vec3){0.0f, 0.0f, 0.0f});
        model_pushNormals(model, normals);
        free(normals.array);
...
void model_printData(const Model model){
        ...
        for(int i = 0; i < model.vertices.size; i++){
                ...
                float nx = model.normals.array[i][0];
                float ny = model.normals.array[i][1];
                float nz = model.normals.array[i][2];
                printf("v%d: (%.2f %.2f %.2f) | nv%d: (%.2f %.2f %.2f)\n",i, x, y, z, i, nx, ny, nz);
        }
        puts("FACES:");
        for(int i = 0; i < model.faces.size; i++){
                printf("%d ", model.faces.array[i*3]);
                printf("%d ", model.faces.array[(i*3)+1]);
                printf("%d ", model.faces.array[(i*3)+2]);
                puts("");
        }
        fflush(stdout);
}
...
void model_clear(Model* model){
        ...
        if(model->normals.array != NULL){
                free(model->normals.array);
                model->normals.size = 0;
        }
}
...
void model_create_sphere(Model* model){
        ...
        for(int i = 0; i < model->vertices.size; i++){
                glm_vec3_normalize(model->vertices.array[i]);
                glm_vec3_copy(model->vertices.array[i], model->normals.array[i]);
        }
}
